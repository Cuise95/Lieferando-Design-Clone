...
function minOrderValue() {
  if (result < 14) {
    document.getElementById(
      "minOrderValue"
    ).innerHTML = `<span>Mindestbestellwert 14€ nicht erreicht</span>`;
    document.getElementById("submitBtn").classList.add("d-none");
  } else {
    document.getElementById("minOrderValue").classList.add("d-none");
...
// Mobile Funktionen
function mobileBtnCreate() {
  if (selectedFood.length > 0) {
    document.getElementById("mobileBtnContainer").innerHTML =
      mobileBtnTemplate();
  }
}
...
function close() {
  return `<span id="closeX" onclick="closeMobileBasket()">x</span>`;
}
// Templates

function mobileShoppingBasketTemplate() {
  return `
  <h3>Warenkorb</h3>
  
  <span>Gesamtsumme${result.toFixed(2)}€</span>
  `;
}

function mobileBtnTemplate() {
  return `<button onclick="openMobileBasket()" id="mobileBtn">Bestellen(${result.toFixed(
    2
  )}€)</button>`;
}
...
function fullMobileBasketTemplate(i) {
  return `
  <div id="selectedFood">
...
<span id="closeMobileBasket" onclick="closeMobileBasket()">X</span>
  `;
}
...
function submitOrderMobileTemplate() {
  return `
  <div class="costsDiv">
  <div id="minOrderValue"></div>
  <div class="subTotal">
  ...
  <button onclick="confirmOrderMobile()" id="submitBtn">
  Bestellen
</button>  
</div>
  `;
...
function submitOrderTemplate() {
  ...
  <button onclick="confirmOrder()" id="submitBtn">
  Bestellen
</button>  
</div>
  `;
...
function confirmationTemplate() {
  return `<div id="confirmDiv" class="confirmationAbsolute d-none">
    ...
    <span>Vielen Dank für deine Bestellung bei FastDelivery</span>
